...
// Build this synthetic credential at runtime so repository secret scanners do
// not mistake the test fixture for a usable key.
const syntheticStripeKey = ["sk", "live", "51Abcdefghijklmnopqrstuvwx"].join("_");
...
describe("client-side exposure", () => {
  ...
  it("catches a literal credential pasted into source", () => {
    const found = clientExposure({
      "src/app.js": `const key = "${syntheticStripeKey}";`,
    });
